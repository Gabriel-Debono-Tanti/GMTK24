...
#include "Lever.hpp"
using namespace std;
class L7 : public Scene
{
private:
     player p = player( {13, 25}, {26*32, 24*32},2);
     TilemapImporter tmp = TilemapImporter("res/Levels/l7.json");
    ...
    DataLoader dl;
    GameData gd;
   StepButton button = StepButton({32, 32}, {15*32, 5*32});
   StepButton button2 = StepButton({32, 32}, {34*32, 5*32});
    Lever lev = Lever({15, 25}, {2*32,18*32});
public:
 L7(/* args */);
    ~L7();
...
void L7::Render(SDL_Renderer* ren)
{ 
